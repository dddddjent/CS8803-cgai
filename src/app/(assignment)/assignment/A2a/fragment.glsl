...
//// You are asked to convert the negative sdf value to a vec4 with the first
/// three components as color rgb values and the last component as density. /
/// For color, you may use the provided palette function to convert the sdf
/// value to an rgb color. / For density, we assume it is alway 1.0 inside the
/// object (sdf
///< 0) and 0.0 outside the object (sdf >= 0).
/////////////////////////////////////////////////////

vec4 readSDFVolume(vec3 p) {
    //// sdf object
    float distance = sdSphere(p - vec3(1.8, 0.0, 0.0), 1.0);

    //// convert sdf value to a color

    //// your implementation starts

    return vec4(palette(distance), distance < 0.0 ? 1.0 : 0.0);
...
//// You are asked to convert the CT data to a vec4 with the first three
/// components as color rgb values and the last component as density. / For
/// density, you should read the density value from the first component of the
/// volumetric texture iVolume with tex_coord. / For color, you should use the
/// provided palette function to convert the density value to an rgb color. /
...
/// algorithm to accummulate colors along each ray. / Your task is to accumulate
/// color and transmittance along the ray based on the absorption-emission
/// volumetric model. / You may want to read the course slides, Equation (3) in
/// the original NeRF paper, and the A2a document for the rendering model.
/////////////////////////////////////////////////////
...
vec4 volumeRendering(vec3 ro, vec3 rd, float near, float far, int n_samples) {
    ...
    for (int i = 0; i < n_samples; i++) {
        ...
        transmittance *= exp(-stepSize * (res1.a + res2.a));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    fragColor = volumeRendering(ro, rd, near, far,
            n_samples); //// volumetric ray marching
}
